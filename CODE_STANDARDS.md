...
1. **DRY is non-negotiable** — Never duplicate logic, configuration, or type definitions. If something exists in more than one place, extract it into a shared function, utility, component, or constant. When you encounter existing duplication, consolidate it as part of your change.
2. **Centralise shared code** — Helpers, API clients, state management, type definitions, and constants belong in a single shared location and must be imported wherever needed. Always check existing utilities, the WPEA component library, and the codebase before creating anything new.
3. **Consistency over preference** — Match existing patterns in the codebase
4. **Explicit over implicit** — Clear, readable code over clever shortcuts
5. **Comments explain "why"** — Code should explain "what"
6. **Fail fast, fail loudly** — Validate early, throw meaningful errors
...
Use `wp_enqueue_script_module()` (WordPress 6.5+) for all Svelte app scripts.
`wp_localize_script()` does NOT work with script modules — pass data via inline scripts instead.

```php
// 1. Output inline theme styles to prevent flash of wrong theme
...
// 2. Enqueue app module (ES module — WordPress handles type="module" and import maps)
...
// 3. Pass data to the module via inline script
...
add_action('admin_print_footer_scripts', static function(): void {
    ...
    wp_print_inline_script_tag('window.{prefix}Data = ' . wp_json_encode($data) . ';');
}, 1);
```

**Key points:**
- `wp_enqueue_script_module()` outputs `<script type="module">` — handles import maps for chunks
- `wp_localize_script()` / `wp_add_inline_script()` do NOT work with script modules
- Use `wp_print_inline_script_tag()` in a footer hook to pass data (modules are deferred)
...
// Custom capabilities (defined in CLAUDE.md)
...
All tables use prefix from CLAUDE.md: `{wp_prefix}_{db_prefix}_tablename`
